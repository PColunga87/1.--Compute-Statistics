...
def compute_sales(price_catalogue, *sales_records_list):
    """
    Compute the total cost for sales based on a price catalogue for multiple sales records.

    Args:
        price_catalogue (str): Path to the JSON file containing price information.
        sales_records_list (list): Paths to the JSON files containing sales records.
    ...
    try:
        ...
        with open('SalesResults2.txt', 'w', encoding='utf-8') as results_file:
                ...
                with open(sales_record, 'r', encoding='utf-8') as sales_file:
                    ...
                    for sale in sales_data:
                        ...
                        if product_name and quantity:
                            matching_products = [
                                product for product in price_data if product.get('title') == product_name
                            ]
                            if matching_products:
                                product_price = matching_products[0].get('price')
                                total_cost += abs(quantity) * product_price
                            else:
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python computeSales.py <price_catalogue.json> <sales_record1.json> <sales_record2.json> ...")
    else:
